...
@kopf.on.update("devsecops.licenta.ro", "v1", "jitaccessrequests")
def on_jit_request_update(
    spec: dict[str, Any],
    old: dict[str, Any],
    new: dict[str, Any],
    status: dict[str, Any],
    ...
    **_: Any,
    ...
    new_spec = new.get("spec", {}) if new else {}
    
    if old_spec.get("requestedRole") != new_spec.get("requestedRole") or old_spec.get("duration") != new_spec.get("duration"):
        target_namespace = spec.get("targetNamespace", "")
        service_account_name = status.get("temporaryServiceAccount")
        rolebinding_name = status.get("roleBindingName")
        ...
        patch.status["state"] = "TAMPERED"
        patch.status["message"] = "Security violation: Mid-flight modification of spec detected. Access revoked."
        ...
        db.record_audit(
            ...
            status="TAMPERED",
            message="Spec tampering detected during active session",
        )
        logger.error("TAMPERING DETECTED for %s in %s. Revoking access immediately.", name, namespace)

@kopf.on.delete("devsecops.licenta.ro", "v1", "jitaccessrequests")
def on_jit_request_delete(
    spec: dict[str, Any],
    status: dict[str, Any],
    name: str,
    namespace: str,
    ...
    **_: Any,
    ...
    state = status.get("state")
    if state not in ("ACTIVE", "TAMPERED"):
        return
    ...
    try:
        ...
        db.record_audit(
            ...
            status="REVOKED",
            message="Manual deletion of JIT request (Kill Switch)",
        )
    except Exception:
        pass
    logger.info("Kill switch invoked. Access for %s revoked on deletion.", name)
